...
class CleanDictObsWrapper(gym.Wrapper):
    ...
    def _to_uint8_rgb(img: np.ndarray) -> np.ndarray:
        ...
        if img.ndim != 3 or img.shape[2] < 3:
            raise ValueError(f"Unexpected cleaned image shape: {img.shape}")
        if img.shape[2] > 3:
            img = img[:, :, :3]
        return img
...
def build_image_state_env_config(image_width: int, image_height: int, training: bool) -> dict:
    ...
    # Lower lidar rendering overhead if image is now the main visual input.
...
def main():
    ...
    eval_env_fns = [
        ...
        for seed in EVAL_VEC_ENV_SEEDS[: max(1, min(len(EVAL_VEC_ENV_SEEDS), args.n_envs))]
    ]
    ...
    if args.resume_from:
        model = SAC.load(args.resume_from, env=env, device="auto")
    ...
    eval_cb = EvalCallback(
        ...
        n_eval_episodes=max(1, len(eval_env_fns)),
        deterministic=True,
        render=False,
    )
