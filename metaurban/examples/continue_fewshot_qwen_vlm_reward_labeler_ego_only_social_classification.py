...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--resume-from-json-dir", action="store_true", default=True)
    parser.add_argument("--no-resume-from-json-dir", action="store_false", dest="resume_from_json_dir")
...
def find_completed_json_indices(json_dir: Path) -> List[int]:
    indices: List[int] = []
    if not json_dir.exists():
        return indices
    for json_path in sorted(json_dir.glob("step_*.json")):
        m = re.fullmatch(r"step_(\d+)\.json", json_path.name)
        if m is not None:
            indices.append(int(m.group(1)))
    return indices


def load_existing_rows_from_json_dir(json_dir: Path) -> List[Dict]:
    rows: List[Dict] = []
    if not json_dir.exists():
        return rows
    for json_path in sorted(json_dir.glob("step_*.json")):
...
def main():
    ...
    if not args.overwrite:
        json_dir_rows = load_existing_rows_from_json_dir(json_dir)
        for row in json_dir_rows:
            idx = int(row["idx"])
            if idx not in done:
                results.append(row)
                done.add(idx)
    ...
    completed_json_indices = find_completed_json_indices(json_dir) if args.resume_from_json_dir else []
    latest_completed_idx = max(completed_json_indices) if completed_json_indices else None

    pending_pairs = [p for p in pairs if p[0] not in done]
    if latest_completed_idx is not None:
        pending_pairs = [p for p in pending_pairs if p[0] > latest_completed_idx]

    if args.verbose:
        if latest_completed_idx is None:
            print(f"Found {len(pairs)} pairs, {len(pending_pairs)} pending, {len(done)} already done.")
        else:
            print(
                f"Found {len(pairs)} pairs, {len(pending_pairs)} pending, "
                f"{len(done)} already done. Resuming after step {latest_completed_idx:06d} based on {json_dir}."
            )
